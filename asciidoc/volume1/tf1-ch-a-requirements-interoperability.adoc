...
[cols="1"]
|===
a| *{standard_note}*: This appendix provides valuable information regarding the "vision" and rationale for key elements of this standard, some of which are novel for any MDI specification.
For previous versions of the Gemini SDPi specification, the content was captured in this appendix; however, it will not be migrated to the SUPPLEMENT document, and that begs whether it should be kept as a Volume 1 appendix or moved to a end-of-standard appendix (after conformance, for example), or somewhere else such as in the SDPi Profiles Overview.
Note that many standards have Appendix A as a rationale and Appendix Z as a Bibliography.
Finally, note that appendices in IHE TF volumes are allocated similarly to profile and transaction numbers, since ultimately they will need to be moved into a single domain technical framework specification.

#REVIEWER QUESTIONS#:

. Should this appendix be kept within Volume 1 or moved the end of the standard as an Appendix A for the entire standard?
. If moved, is there another location than specified above that might be best for this information (e.g., in the front-matter somewhere)?
...
Some are technology focused, such as the IEEE 11073 standards for health device communication, and others are more focused on quality and risk management, such as the ISO/IEC 80001-1 and ISO 14971 standards.
...
. Standards are sometimes grouped into families (e.g., IEEE 11073 SDC) - this indicates that they are internally cohesive as well as able to be integrated as a group with other areas;
...
. Leverage the IEEE 11073-1070x <<term_participant_key_purposes>> standards, which represent a consensus standard for risk management of technologies that are implemented in that left-right gap on the Temple model;
...
NOTE: This section includes guidance and requirements that are focused on unique *_Safety_* requirements associated with the relevant technical framework element.  Note: a simple definition of safety within the context of risk management is "freedom from unacceptable harm" (see https://81001.org/concept/safety[81001.org/safety])
...
NOTE: This section includes guidance and requirements that are focused on unique *_Effectiveness_* requirements associated with the relevant technical framework element.  Note:  in the context of risk management key properties, effectiveness is the ability to perform the intended use (see https://81001.org/concept/effectiveness[81001.org/effectiveness])
...
NOTE: This section includes guidance and requirements that are focused on unique *_Security_* requirements associated with the relevant technical framework element.
...
[cols="1"]
|===
a| *{standard_note}*: The information in this section includes both general requirements modeling information that captures the metadata that is ultimately exported for document-external use.
...
See <<vol1_figure_appendix_a_sdpi_requirements_core_model>> below for possible pathways for fully achieving the vision above.
...
| SES
| Non-technical requirements related to Safety, Effectiveness, and Security are captured in these blocks.  These are especially relevant to mapping IEEE 11073-1070x Participant Key Purposes standard requirements to elements within the SDPi specification.
...
| Requirement OID
| An ISO Object Identifier that assigned from the <<vol3_appendix_c_oid_requirements,SDPi requirement OID hierarchy>> for the specific requirement.
| not applicable; assigned automatically
| `{SDPi-parent-oid}.10.1001`
...
// currently (Feb 2026) not supported
| Requirement Parent 
...
a|
* Unique identifiers may be used to link requirements
* This is differentiated from a Requirement Fulfilment
...
* *Unique Identifier*
** Human-readable text that is displayed with detailed requirement content in the specification, for example `R1234`.
** Identifiers begin with a capital `R` and have a simple four digit numeric value.
** Identifiers are typically assigned in groups to related content for convenience (e.g., requirements for halloween may be assigned R1301, R1302 and R1303), however no ontology or taxonomy shall be inferred from requirement numbering.

* *Requirement OID*
** A machine-readable URI style identifier that ensures global uniqueness.
** The <<vol3_appendix_c_oid_requirements,root OID>> for all requirements in the SDPi specification is `{sdpi-parent-oid}.10`.
** The numeric part of the Unique Identifier is the final arc for the OID, for example: `{sdpi-parent-oid}.10.1234`. 

NOTE: A complete list of OIDs for requirements defined in this specification is provided in <<vol3_appendix_c_oid_requirements>>.

FOR THE CURRENT SPECIFICATION:

* The _unique identifier_ is assigned manually; the asciidoc processor checks all identifiers are unique within the document and automatically assigns the _requirement OID_.
* Requirements are *not* versioned.
* A new _unique identifier_ is assigned following material changes to a requirements normative statement and the old identifier is retired, never to be used again.
* Assignment may evolve with experience and extended use. 

Material modifications, requiring a new _unique identifier_, include those that may:

* alter the actors referenced in the normative statement,
* affect conformant implementations,
* affect testing testing and validation, or
* impact interoperability (e.g., a test or implementation conforming to the old requirement text may not be conformant to the new one).

Changes that are not material and *do not* require a new _unique identifier_ include:

* improving the grammar and/or clarity of the requirement's normative statement,
* changing a requirement level (may &rArr; should; shall &rArr; should) because this only impacts implementation obligations but not the purpose of the requirement,
* changes to notes, examples or related requirements, or
* changes that may make a non-conformant implementation and/or test conformant (provided the change won't affect conformant implementations and/or tests).

NOTE: changes requiring assignment of new requirement identifiers may evolve with experience and/or finalization of the profile.
...
Given the rule above, bindings are made in the current <<vol2_appendix_a_mdpws_messages>> specification pointing backward (or upward!) to the transaction requirements that they satisfy.
...
Application of this rule would also hold true in other places such as backward references from a profile's Use Case section to the specific <<clause_sdpi_use_cases>> use case and scenario requirements that they satisfy.
...
[cols="1"]
|===
a| *{standard_note}*: Additional detail will be added in a subsequent version.  Additional metadata may include:
