...
[cols="1"]
|===
a| *{standard_note}*:  This version of the <<acronym_sdpi_r>> Profile is built upon the foundational <<acronym_sdpi_p>> Profile but does not provide substantially more capabilities.
This is due to the fact that the primary purpose of this <<acronym_sdpi_r>> Profile, namely communication of medical data to accomplish intended medical purposes, requires the full integration of two emerging IEEE standards:  <<ref_ieee_11073_10700_2022>> and <<ref_ieee_11073_10701_2022>>.
Their requirements will be integrated into this standard, with their <<term_implementation_conformance_statement>> added to <<clause_references_referenced_standards_conformance>> below.
Many of those requirements will be mapped to the actors and transactions and other elements in this standard, including this <<acronym_sdpi_r>> Profile.

Additionally, though the <<vol1_spec_sdpi_r_actor_somds_dec_gateway>> is defined below and fully specified in <<vol2_clause_appendix_sdpi_dec_gateway>>, the implementation guide for mapping from <<acronym_biceps>> to <<acronym_hl7>> <<acronym_fhir>> remains in development, pushing the specification of the <<vol1_spec_sdpi_r_actor_somds_fhir_medical_data_gateway>> to a later version of this standard.
...
. Addressing requirements from the emerging <<acronym_pkp>> IEEE standards:  <<ref_ieee_11073_10700_2022>> and <<ref_ieee_11073_10701_2022>>
...
NOTE: see <<vol1_clause_sdpi_r_required_actor_groupings>> for actor inheritance and grouping.
...
// Old (manual) version:
include::actor-tables/sdpi-r-transactions.adoc[]
...
Gateway directionality (export, import, or both/bidirectional) is specified by the <<vol1_spec_sdpi_p_option_gateway_import>> and <<vol1_spec_sdpi_p_option_gateway_export>> in the <<vol1_clause_sdpi_p_profile_reftext>>.
For the <<vol1_spec_sdpi_r_actor_somds_dec_gateway>>, the following configurations shall apply:

[#vol1_table_sdpi_r_dec_gateway_direction]
.DEC Gateway Direction Requirements
[%autowidth]
[cols="1,1,1,2"]
|===
.^|Direction
...
| SDC-to-DEC
| DOR
| Gateway retrieves information from <<vol1_spec_sdpi_r_actor_somds_medical_data_provider>> systems, converts and publishing them as [DEV-01] / [PCD-01] Transactions to external DEC Device Observation Consumer (DOC) systems.

| import
| DEC-to-SDC
| DOC
| Gateway receives [DEV-01] / [PCD-01] Transactions from external DEC Device Observation Reporter (DOR) systems and makes them available to other to  <<vol1_spec_sdpi_r_actor_somds_medical_data_consumer>> systems.

| bidirectional
| SDC-to-DEC + DEC-to-SDC
| DOR + DOC
| Gateway supports both export and import functions to external systems.
...
[actor-grouping="somds_fhir_gateway"]
...
[cols="1"]
|===
a| *{standard_note}*:  The HL7 FHIR resources and related Point-of-Care Device FHIR Implementation Guide (PoCD FHIR IG) is still under active development.
...
When the FHIR specifications are finalized, then this actor will be fully specified in a future SDPI standard version.
...
// [cols="1"]
// |===
// a| *{standard_note}*:  This section is left intentionally blank to indicate capabilities that will be added in a future version of the SDPi standard.
// 
// [#vol1_clause_sdpi_r_actor_option_retrieve_remote_data_reftext, reftext='SDPi-R Option: Retrieve Remote Data']
// [%noheader]
// [%autowidth]
...
An actor from this profile (Column 1) shall implement all of the required transactions and/or content modules in this profile *_in addition to all_* of the requirements for the grouped actor (Column 3) in <<vol1_table_sdpi_r_required_actor_groupings>>.
...
<<vol1_clause_sdpi_r_ses_considerations>> describes some optional groupings that may be of interest for safety, effectiveness and security considerations and <<vol1_clause_sdpi_r_cross_profile_considerations>> describes some optional groupings in other related profiles.

[#vol1_table_sdpi_r_required_actor_groupings]
.SDPi-R - Required Actor Groupings
[%autowidth]
[cols="1,1,1,1"]
|===
...
| <<vol1_spec_sdpi_r_actor_somds_dec_gateway>>
| Required
| <<vol1_spec_sdpi_p_actor_somds_v2_gateway>>
| <<vol1_clause_sdpi_r_somds_dec_gateway>>

| <<vol1_spec_sdpi_r_actor_somds_fhir_medical_data_gateway>>
| Required
...
[cols="1"]
|===
a| *{standard_note}*:  An overview of the concepts for this <<vol1_clause_sdpi_r_profile_reftext>> will be provided in a future SDPi standard version.
...
==== Use Cases
The SDPi-R Profile supports requirements from use cases detailed in <<clause_sdpi_use_cases>>.  The following subsections identify the specific use case requirements that are fulfilled with capabilities provided by this specification.
...
* *Scenarios*: <<acronym_sicdsp>> <<clause_use_case_sicdsp_scenarios>> communication of medical data to a <<vol1_spec_sdpi_p_actor_somds_consumer>> <<system_type_dashboard>>

sdpi_support_use_case::somds_medical_data_consumer[support=optional]
...
* *System Type*:  N/A
* *Technical Pre-Conditions*: N/A
* *Scenarios*: <<acronym_sicdmp>> <<clause_use_case_sicdmp_scenarios>> communication of medical data to a <<vol1_spec_sdpi_p_actor_somds_consumer>> <<system_type_dashboard>>

sdpi_support_use_case::somds_medical_data_consumer[support=optional]

[role="support-use-case", use-case-id="ddes", oid-arcs=.3]
...
* *Service Type*: N/A
* *Technical Pre-Conditions*: N/A
* *Scenarios*: <<acronym_ddes>> <<clause_use_case_ddes_scenarios>> communication of medical data to a <<vol1_spec_sdpi_p_actor_somds_consumer>> <<system_type_device_gateway>>

////
#TODO:  Reference DEC Gateway / DOR actor from generic use case scenario reference#
////

sdpi_support_use_case::somds_dec_gateway[support=optional]
sdpi_support_use_case::somds_fhir_medical_data_gateway[support=optional]
...
[cols="1"]
|===
a| *{standard_note}*:  The <<ref_ieee_11073_10700_2022>> and <<ref_ieee_11073_10701_2022>> standards are now published by the IEEE.
However, their requirements are yet to be integrated into this standard, with many of them being mapped to elements in this <<acronym_sdpi_r>> Profile.
