...
[cols="1"]
|===
a| *{standard_note}*: Some actors and transactions have been deferred to a subsequent version, but are included here for completeness.
Specifically:  SOMDS Sensor Gateway and SOMDS Smart App Platform have been deferred.
...
NOTE: “Receiver” is indicated for actors that receive but do not directly respond to a specific transaction.

sdpi_include_transaction::DEV-23[initiator="required", receiver="optional"]
sdpi_include_transaction::DEV-24[initiator="required", responder="required"]
sdpi_include_transaction::DEV-25[initiator="required", responder="required"]
sdpi_include_transaction::DEV-27[initiator="required", responder="required"]
sdpi_include_transaction::DEV-28[initiator="optional", receiver="optional"]
sdpi_include_transaction::DEV-29[initiator="required", receiver="required"]
sdpi_include_transaction::DEV-30[initiator="optional", responder="optional"]
sdpi_include_transaction::DEV-32[initiator="optional", responder="optional"]
sdpi_include_transaction::DEV-33[initiator="optional", responder="optional"]

sdpi_include_transaction::DEV-46[actor-id=somds_provider,initiator=optional]
sdpi_include_transaction::DEV-47[actor-id=somds_consumer,initiator=optional]

// Deferred transactions. 
sdpi_include_transaction::DEV-26[actor-id="somds_provider",responder="required",placeholder-name="Discover system context and capabilities"]
sdpi_include_transaction::DEV-31[actor-id="somds_provider",responder="optional",placeholder-name="Set provider state"]
...
sdpi_include_transaction::DEV-31[actor-id="somds_consumer",initiator="optional",placeholder-name="Set provider state"]

ifdef::markup-actor-transactions-table[]

[#vol1_table_sdpi_p_actors_transactions]
.SDPi-P Profile - Actors and Transactions
sdpi_transaction_table::[profile-id="sdpi-p"]

[NOTE]
====
* RefTransaction:DEV-28[]  is required if there are 
dynamic changes that may need to be sent to subscribing systems

* _“Receiver”_ indicates that though a <<vol1_spec_sdpi_p_actor_somds_participant>> 
may "receive" the transaction, there is no response communicated to the message initiator.

* A future SDPi version will provide transaction details for the 
RefActor:somds_connector[], RefActor:somds_fhir_gateway[], 
RefActor:somds_v2_gateway[], RefActor:somds_sensor_gateway[], 
RefActor:somds_smart_app[], RefActor:biceps_content_creator[], and
RefActor:biceps_content_consumer[] actors. 

* See <<vol1_clause_sdpi_p_required_actor_groupings>> for actor inheritance and grouping. 
====
endif::[]
...
include::actor-tables/sdpi-p-transactions.adoc[]
...
In general, the <<vol1_spec_sdpi_p_actor_somds_provider>> will create content for consumption by a <<vol1_spec_sdpi_p_actor_somds_consumer>> , but the communication cloud between the two actors indicates that the technical method of exchange is a separate concern from the semantic content.
...
A product implementation using this specification may group actors from this profile with actors from a workflow or transport profile to be functional.
...
sdpi_content_module_table::[profile-id="sdpi-p"]

NOTE: _All content exchanged on a SOMDS shall conform to the general SDPi “BICEPS Content Module” 
requirements (see <<vol3_clause_sdc_biceps_semantic_content>>). 
<<vol1_spec_sdpi_p_actor_somds_provider>>-specific content modules (e.g., infusion pumps) may be 
optionally supported as indicated._

// Old (manual) version:
// include::actor-tables/sdpi-p-content-modules.adoc[]
...
[#vol1_clause_sdpi_p_somds_participant,role="actor actor-alias",actor-id="somds_participant",oid-arcs=.40]
===== SOMDS Participant
...
This required grouping ensures that all systems connecting to a SOMDS network support the <<acronym_ses_mdi>>  requirements footnote:[See <<vol1_appendix_a_requirements_management_for_p_n_t_interperability>> and <<vol1_clause_ses_considerations_requirements>> for more detail on <<acronym_ses_mdi>>] necessary for establishing a <<term_plug_and_trust>> ecosystem, including the secure and dynamic provision of an implementation’s <<term_system_function_contribution>>.
...
[#vol1_clause_sdpi_p_somds_provider,role="actor actor-alias",actor-id="somds_provider",actor-grouping=somds_participant,oid-arcs=.41]
===== SOMDS Provider
...
[#vol1_clause_sdpi_p_somds_consumer,role="actor actor-alias",actor-id="somds_consumer",actor-grouping=somds_participant,oid-arcs=.30]
===== SOMDS Consumer
...
===== SOMDS Connector
[#vol1_spec_sdpi_p_actor_somds_connector, reftext='SOMDS Connector',role=actor-alias,actor-id="somds_connector"]
Actor Summary Definition:
[none]
...
[cols="1"]
|===
a| *{standard_note}*: The TF-3 SDC/BICEPS Mapping of SOMDS Connector Content Modules section is out-of-scope, but included above for completeness of this actor overview.
|===
...
[#vol1_clause_sdpi_p_somds_v2_gateway,role="actor actor-alias",actor-id="somds_v2_gateway",oid-arcs=.44]
[actor-grouping="somds_consumer[sdpi-p-gateway-direction-export],somds_provider[sdpi-p-gateway-direction-import]"]
===== SOMDS V2 Gateway
[#vol1_spec_sdpi_p_actor_somds_v2_gateway, reftext='SOMDS V2 Gateway',role=actor-alias,actor-id="somds_v2_gateway"]
Actor Summary Definition:
[none]
...
[#vol1_clause_sdpi_p_somds_sensor_gateway,role="actor actor-alias",actor-id="somds_sensor_gateway",oid-arcs=.42]
===== SOMDS Sensor Gateway
[%noheader]
[%autowidth]
[cols="1"]
|===
| *{standard_note}*: Detailed specifications for this actor are deferred to a later version of the SDPi standard.
|===

[#vol1_spec_sdpi_p_actor_somds_sensor_gateway, reftext='SOMDS Sensor Gateway',role=actor-alias,actor-id="somds_sensor_gateway"]
Actor Summary Definition:
[none]
...
[#vol1_clause_sdpi_p_somds_smart_app_platform,role="actor actor-alias",actor-id="somds_smart_app",oid-arcs=.43]
...
[#vol1_spec_sdpi_p_actor_somds_smart_app_platform, reftext='SOMDS Smart App Platform',role=actor-alias,actor-id="somds_smart_app"]
Actor Summary Definition:
[none]
...
As a _<<vol1_spec_sdpi_p_actor_somds_connector>>_, the <<vol1_spec_sdpi_p_actor_somds_smart_app_platform>> Actor maintains a 1:1 connection with other <<vol1_spec_sdpi_p_actor_somds_provider>> and <<vol1_spec_sdpi_p_actor_somds_consumer>> systems, but can simultaneously support multiple (1:* multiplicity) apps that utilize its services. Without the <<vol1_spec_sdpi_p_actor_somds_smart_app_platform>> Actor, each app would have to implement separate connections to each _<<vol1_spec_sdpi_p_actor_somds_participant>>_, resulting in increased network traffic and communication processing overhead.
...
[#vol1_clause_sdpi_p_biceps_content_creator,role="actor actor-alias",actor-id="biceps_content_creator",oid-arcs=.27]
...
sdpi_include_content_module::biceps[actor-id="biceps_content_creator",support="required"]
sdpi_include_content_module::infusion-pump[actor-id="biceps_content_creator",support="optional"]
sdpi_include_content_module::ventilator[actor-id="biceps_content_creator",support="optional"]
sdpi_include_content_module::physiological-monitor[actor-id="biceps_content_creator",support="optional"]
sdpi_include_content_module::surgical[actor-id="biceps_content_creator",support="optional"]
sdpi_include_content_module::anesthesia[actor-id="biceps_content_creator",support="optional",placeholder-name="Anesthesia devies SDC/BICEPS content module"]
sdpi_include_content_module::dialysis[actor-id="biceps_content_creator",support="optional",placeholder-name="Dialysis devices SDC/BICEPS content module"]



[#vol1_clause_sdpi_p_biceps_content_consumer,role="actor actor-alias",actor-id="biceps_content_consumer",oid-arcs=.26]
...
A <<vol1_spec_sdpi_p_actor_biceps_content_consumer>> shall be capable of processing information provided by a <<vol1_spec_sdpi_p_actor_biceps_content_creator>>, in accordance to the BICEPS content module specifications in <<vol3_clause_sdc_biceps_semantic_content_module>> and related sections.
...
For robustness, a <<vol1_spec_sdpi_p_actor_biceps_content_consumer>> need only process the content that is necessary to support its capabilities, but shall also be able to accept any additional content that may be provided but is out-of-scope for its internal requirements. footnote:[Apply Postel’s Law:  Send conservatively, Accept liberally.]
...
sdpi_include_content_module::biceps[actor-id="biceps_content_consumer",support="required"]
sdpi_include_content_module::infusion-pump[actor-id="biceps_content_consumer",support="optional"]
sdpi_include_content_module::ventilator[actor-id="biceps_content_consumer",support="optional"]
sdpi_include_content_module::physiological-monitor[actor-id="biceps_content_consumer",support="optional"]
sdpi_include_content_module::surgical[actor-id="biceps_content_consumer",support="optional"]
sdpi_include_content_module::anesthesia[actor-id="biceps_content_consumer",support="optional",placeholder-name="Anesthesia devies SDC/BICEPS content module"]
sdpi_include_content_module::dialysis[actor-id="biceps_content_consumer",support="optional",placeholder-name="Dialysis devices SDC/BICEPS content module"]


[#vol1_clause_sdpi_p_discovery_proxy,role="actor actor-alias",actor-id="discovery_proxy",oid-arcs=.32]
===== Discovery Proxy

[#vol1_spec_sdpi_p_actor_discovery_proxy, reftext='Discovery Proxy',role=actor-alias,actor-id="discovery_proxy"]
Actor Summary Definition:
[none]
...
Options that may be selected for this Integration Profile are listed in the <<vol1_table_sdpi_p_actors_options>> along with the actors to which they apply.
Dependencies between options when applicable are specified in
notes.

[#vol1_table_sdpi_p_actors_options]
...
| <<vol1_spec_sdpi_p_option_gateway_import>> +
  <<vol1_spec_sdpi_p_option_gateway_export>>
| <<vol1_clause_sdpi_p_gateway_direction>>

| <<vol1_spec_sdpi_p_actor_somds_fhir_gateway>>
| <<vol1_spec_sdpi_p_option_gateway_import>> +
  <<vol1_spec_sdpi_p_option_gateway_export>>
| <<vol1_clause_sdpi_p_gateway_direction>>
...
[#vol1_clause_sdpi_p_option_managed_discovery,role=profile-option,profile-option-id="sdpi-p-managed-discovery",oid-arcs=".1"]
==== Managed Discovery Option
[#vol1_spec_sdpi_p_option_managed_discovery, reftext='Managed Discovery Option']
...
[plantuml, target=puml-sdpi-p-managed-discovery-option-sequence-diagram, format=svg, format=svg]
....
...
sdpi_include_transaction::DEV-46[initiator="required", receiver="required"]
...
[NORMATIVE]
====
When the RefProfile:sdpi-p[profile-option-id="sdpi-p-managed-discovery"] is enabled for a RefActor:somds_provider[] actor, then it shall use the RefTransaction:DEV-46[] transaction to update the RefActor:discovery_proxy[] actor on its network presence and departure.
====

[RELATED]
====
* <<ref_oasis_ws_discovery_2009>>, §2.2.2, Managed mode.
====

****

.R1022
[sdpi_requirement#r1022,sdpi_req_level=shall,sdpi_req_type=tech_feature]
****

[NORMATIVE]
====
If a <<vol1_spec_sdpi_p_actor_somds_provider>> Actor is configured or provisioned for the <<vol1_spec_sdpi_p_option_managed_discovery>>, but the proxy system is not available, then the <<vol1_spec_sdpi_p_actor_somds_provider>> shall revert back to "ad hoc" discovery mode.
====

****

.R1023
[sdpi_requirement#r1023,sdpi_req_level=shall,sdpi_req_type=tech_feature]
****

[NORMATIVE]
====
When the RefProfile:sdpi-p[profile-option-id="sdpi-p-managed-discovery"] is enabled for a RefActor:somds_consumer[] actor, then it shall use the RefTransaction:DEV-47[] transaction to retrieve RefActor:somds_provider[] network presence metadata from the  RefActor:discovery_proxy[] actor.
====

[NOTE]
====
* When retrieving network presence metadata from a <<vol1_spec_sdpi_p_actor_discovery_proxy>> Actor, a <<term_discovery_scope>> may be specified as a filter to identify a specific subset of <<vol1_spec_sdpi_p_actor_somds_provider>> systems.

* A <<vol1_spec_sdpi_p_actor_somds_consumer>> may optionally use the <<vol2_clause_dev_47>> transaction to subscribe to all metadata updates from a set of <<vol1_spec_sdpi_p_actor_somds_consumer>> systems, essentially using the <<vol1_spec_sdpi_p_actor_discovery_proxy>> Actor as a pass through for <<vol1_spec_sdpi_p_actor_somds_provider>> <<vol2_clause_dev_23>> transactions.
====
****

.R1024
[sdpi_requirement#r1024,sdpi_req_level=shall,sdpi_req_type=tech_feature]
****

[NORMATIVE]
====
If a <<vol1_spec_sdpi_p_actor_somds_consumer>> Actor is configured or provisioned for the <<vol1_spec_sdpi_p_option_managed_discovery>>, but the proxy system is not available, then the <<vol1_spec_sdpi_p_actor_somds_consumer>> shall revert back to "ad hoc" discovery mode.
====
****
...
[NOTE]
====
For example, issuing a periodic "heartbeat" check message or cable-connected check.
Specification of these means will be provided for in the transport-specific implementation specification for the RefTransaction:DEV-46[] transaction.
====
****
...
The Gateway Direction specifies the flow of information through a gateway actor.
If one of the Gateway Direction options (<<vol1_spec_sdpi_p_option_gateway_import,import>> or <<vol1_spec_sdpi_p_option_gateway_export,export>>) is not configured, then the DEFAULT 
direction of any gateway actors shall be "<<vol1_spec_sdpi_p_option_gateway_export,export>>". 

.R1040
[sdpi_requirement#r1040,sdpi_req_level=shall,sdpi_req_type=tech_feature]
****

[NORMATIVE]
====
Each gateway actor, which is an instance of the abstract 
<<vol1_spec_sdpi_p_actor_somds_connector>> Actor, shall be grouped with either
a RefActor:somds_consumer[] Actor or RefActor:somds_provider[] Actor or both. 
====

****

[#vol1_spec_sdpi_p_option_gateway_export, reftext='Gateway Export Direction Option']
[role=profile-option,profile-option-id="sdpi-p-gateway-direction-export",oid-arcs=".2"]
===== Gateway Export Direction Option

The Gateway Export Direction Option specifies the flow of information from
<<vol1_spec_sdpi_p_actor_somds_provider>> systems and sends it to other
systems, including those outside the SOMDS.  

.R1041
[sdpi_requirement#r1041,sdpi_req_level=shall,sdpi_req_type=tech_feature]
****

[NORMATIVE]
====
Each gateway actor instance of the abstract 
<<vol1_spec_sdpi_p_actor_somds_connector>> Actor, that selects the
Gateway Export Direction Option shall be grouped with a RefActor:somds_consumer[] Actor. 
====

****

// Transaction obligations for gateway export direction option. 
sdpi_include_transaction::DEV-23[actor-id=somds_connector,receiver=optional]
sdpi_include_transaction::DEV-24[actor-id=somds_connector,initiator=required]
sdpi_include_transaction::DEV-25[actor-id=somds_connector,initiator=required]
sdpi_include_transaction::DEV-27[actor-id=somds_connector,initiator=required]
sdpi_include_transaction::DEV-28[actor-id=somds_connector,receiver=optional]
sdpi_include_transaction::DEV-29[actor-id=somds_connector,receiver=required]
sdpi_include_transaction::DEV-30[actor-id=somds_connector,initiator=optional]
sdpi_include_transaction::DEV-33[actor-id=somds_connector,initiator=optional]
sdpi_include_transaction::DEV-47[actor-id=somds_connector,initiator=optional]


[#vol1_spec_sdpi_p_option_gateway_import, reftext='Gateway Import Direction Option']
[role=profile-option,profile-option-id="sdpi-p-gateway-direction-import",oid-arcs=".3"]
===== Gateway Import Direction Option

The Gateway Import Direction Option specifies the flow of information from
external systems to <<vol1_spec_sdpi_p_actor_somds_consumer>> systems.  

.R1042
[sdpi_requirement#r1042,sdpi_req_level=shall,sdpi_req_type=tech_feature]
****
...
Each gateway actor instance of the abstract 
<<vol1_spec_sdpi_p_actor_somds_connector>> Actor, that selects the
Gateway Import Direction Option shall be grouped with a RefActor:somds_provider[] Actor. 
====

****

// Transaction obligations for gateway import direction option. 
sdpi_include_transaction::DEV-23[actor-id=somds_connector,initiator=required]
sdpi_include_transaction::DEV-24[actor-id=somds_connector,responder=required]
sdpi_include_transaction::DEV-25[actor-id=somds_connector,responder=required]
sdpi_include_transaction::DEV-27[actor-id=somds_connector,responder=required]
sdpi_include_transaction::DEV-28[actor-id=somds_connector,initiator=optional]
sdpi_include_transaction::DEV-29[actor-id=somds_connector,initiator=required]
sdpi_include_transaction::DEV-30[actor-id=somds_connector,responder=optional]
sdpi_include_transaction::DEV-33[actor-id=somds_connector,responder=optional]
sdpi_include_transaction::DEV-46[actor-id=somds_connector,initiator=optional]



[#vol1_clause_sdpi_p_option_real_time_waveforms]
==== Real Time Waveforms Option
[#vol1_spec_sdpi_p_option_real_time_waveforms, reftext='Real Time Waveforms Option']


The SDC Real Time Waveforms option indicates that an <<vol1_spec_sdpi_p_actor_somds_provider>> Actor or <<vol1_spec_sdpi_p_actor_somds_consumer>> Actor support the exchange of real time waveform information.
When this option is selected for one of these actors, the following Transactions shall be supported:

[#vol1_table_sdpi_p_option_real_time_waveform_required_transactions]
.SDPi-P - SDC Real Time Waveform Transactions
[%autowidth]
[cols="^1,1,1,1,^1"]
|===
.^|Transaction
...
| R
| Indicates if the WAVEFORM SERVICE is available.
| <<vol2_clause_dev_25>>

| DEV-27
| <<transaction_name_manage_biceps_subscription>>
| R
| Subscription to waveform stream(s)
| <<vol2_clause_dev_27>>

| DEV-28
| <<transaction_name_notify_change_in_system_context_and_capabilities>>
| R
| Indicates if there are changes in waveform-related stream information
| <<vol2_clause_dev_28>>

| DEV-29
| <<transaction_name_publish_biceps_update_reports>>
| R
| Provides waveform stream content updates
| <<vol2_clause_dev_29>>

5+<|
Note 1:  For optionality of the above transactions for the provider and consumer actors, see <<vol1_table_sdpi_p_actors_transactions>>.
...
older versions of a <<vol1_spec_sdpi_p_actor_somds_provider>>s 
<<acronym_mdib>> than the current version available through 
RefTransaction:DEV-30[]. This may be useful to fill in gaps in
<<acronym_mdib>> versions arising from, for example, network
connectivity interruptions, etc. 

sdpi_include_transaction::DEV-32[initiator="required", responder="required"]


.R1026
[sdpi_requirement#r1026,sdpi_req_level=shall,sdpi_req_type=tech_feature]
****

[NORMATIVE]
====
RefActor:somds_provider[] actor shall implement RefTransaction:DEV-32[] to
deliver historic <<acronym_mdib>> data. 
====

[NOTE]
====
The <<term_history_service>> replaces the <<ref_ieee_11073_20701_2018>> archive service.
====
...
An actor from this profile (Column 1) shall implement all of the required transactions and/or content modules in this profile *_in addition to all_* of the requirements for the grouped actor (Column 3) in <<vol1_table_sdpi_p_required_actor_groupings>>.
...
<<vol1_clause_sdpi_p_ses_considerations>> describes some optional groupings that may be of interest for safety, effectiveness and security considerations and <<vol1_clause_sdpi_p_cross_profile_considerations>> describes some optional groupings in other related profiles.

[#vol1_table_sdpi_p_required_actor_groupings]
.SDPi-P - Required Actor Groupings
[%autowidth]
[cols="1,1,1,1"]
|===
...
.3+| <<vol1_spec_sdpi_p_actor_somds_fhir_gateway>>
| <<vol1_spec_sdpi_p_option_gateway_export>>
| <<vol1_spec_sdpi_p_actor_somds_consumer>>
| <<vol1_clause_sdpi_p_gateway_direction>>
...
| <<vol1_spec_sdpi_p_option_gateway_export>> and <<vol1_spec_sdpi_p_option_gateway_import>> (bidirectional)
| <<vol1_spec_sdpi_p_actor_somds_provider>> + <<vol1_spec_sdpi_p_actor_somds_consumer>>
| <<vol1_clause_sdpi_p_gateway_direction>>

.3+| <<vol1_spec_sdpi_p_actor_somds_v2_gateway>>
| <<vol1_spec_sdpi_p_option_gateway_export>>
| <<vol1_spec_sdpi_p_actor_somds_consumer>>
| <<vol1_clause_sdpi_p_gateway_direction>>
...
| <<vol1_spec_sdpi_p_actor_biceps_content_creator>>
| Required (See Note 1)
| <<vol1_spec_sdpi_p_actor_somds_provider>>
| <<vol1_clause_sdpi_p_biceps_content_creator>>

| <<vol1_spec_sdpi_p_actor_biceps_content_consumer>>
| Required (See Note 1)
| <<vol1_spec_sdpi_p_actor_somds_consumer>>
| <<vol1_clause_sdpi_p_biceps_content_consumer>>

4+<|
Note 1: _The <<vol1_spec_sdpi_p_actor_biceps_content_creator>> and <<vol1_spec_sdpi_p_actor_biceps_content_consumer>> groupings above apply to all content modules listed in <<vol1_table_sdpi_p_actors_content_modules>>._
...
The SDPi-P Profile supports requirements from use cases detailed in <<clause_sdpi_use_cases>>.  The following subsections identify the specific use case requirements that are fulfilled with capabilities provided by this specification.
...
[role="support-use-case", use-case-id="stad", oid-arcs=.1]
===== <<label_use_case_name_stad>> (<<acronym_stad>>)
This use case fully addresses the requirements from  << >>.
...
////
* *Technical Pre-Conditions*:  <<acronym_stad>> <<vol1_clause_sdpi_use_case_stad_technical_precondition>> are fully supported by SDPi-P
* *Scenarios*:  <<acronym_stad>> <<vol1_clause_sdpi_usee_case_stad_scenarios>> are fully supported by SDPi-P
...
sdpi_support_use_case::somds_provider[support=required]
sdpi_support_use_case::somds_consumer[support=required]
sdpi_support_use_case::somds_connector[support=required]
sdpi_support_use_case::somds_fhir_gateway[support=required]
sdpi_support_use_case::somds_v2_gateway[support=required]
sdpi_support_use_case::somds_sensor_gateway[support=required]
sdpi_support_use_case::somds_smart_app[support=required]
sdpi_support_use_case::biceps_content_creator[support=required]
sdpi_support_use_case::biceps_content_consumer[support=required]
sdpi_support_use_case::discovery_proxy[support=required]

[role="support-use-case", use-case-id="sicdsp", oid-arcs=.2]
===== <<label_use_case_name_sicdsp>> (<<acronym_sicdsp>>)
This use case provides capabilities for requirements from <<clause_use_case_sicdsp>>.
...
* *Technical Pre-Conditions*: <<acronym_sicdsp>> <<clause_use_case_sicdsp_technical_precondition>> are fully supported by SDPi-P
* *Scenarios*: <<acronym_sicdsp>> <<clause_use_case_sicdsp_scenarios>> basic communication requirements are supported by SDPi-P
...
sdpi_support_use_case::somds_consumer[support=optional]
sdpi_support_use_case::somds_connector[support=optional]
sdpi_support_use_case::somds_smart_app[support=optional]
sdpi_support_use_case::biceps_content_consumer[support=optional]

[role="support-use-case", use-case-id="sicdmp", oid-arcs=.3]
===== <<label_use_case_name_sicdmp>> (<<acronym_sicdmp>>)
This use case provides capabilities for requirements from  <<clause_use_case_sicdmp>>.

Specific capabilities supporting the <<acronym_sicdmp>> use case include:

* *System Type*:  <<acronym_md_lan>> supported by SDPi-P <<acronym_pnt>> capabilities (see <<vol1_figure_sdpi_p_example_sequence_diagram>>)
* *System Type*:  <<system_type_dashboard>> is supported by the BICEPS Content Module Systems Types Nomenclature (see <<vol1_table_system_type_nomenclature_extensions>>)
* *Technical Pre-Conditions*: <<acronym_sicdmp>> <<clause_use_case_sicdmp_technical_precondition>> are fully supported by SDPi-P
* *Scenarios*: <<acronym_sicdmp>> <<clause_use_case_sicdmp_scenarios>> basic communication requirements are supported by SDPi-P
...
[role="support-use-case", use-case-id="ddes", oid-arcs=.4]
===== <<label_use_case_name_ddes>> (<<acronym_ddes>>)
This use case provides capabilities for requirements from  <<clause_use_case_ddes>>.
...
* *Technical Pre-Conditions*: <<acronym_ddes>> <<clause_use_case_ddes_technical_precondition>> are fully supported by SDPi-P
* *Scenarios*: <<acronym_ddes>> <<clause_use_case_ddes_scenarios>> basic communication requirements are supported by SDPi-P
...
[role="support-use-case", use-case-id="acns", oid-arcs=.5]
===== <<label_use_case_name_acns>> (<<acronym_acns>>)
This use case provides capabilities for requirements from  <<clause_use_case_acns>>.
...
* *Technical Pre-Conditions*:  <<acronym_acns>> <<clause_use_case_acns_technical_precondition>> are fully supported by SDPi-P
* *Scenarios*:  <<acronym_acns>> <<clause_use_case_acns_scenarios>> basic communication requirements are supported by SDPi-P

sdpi_support_use_case::somds_connector[support=optional]
sdpi_support_use_case::somds_fhir_gateway[support=optional]
sdpi_support_use_case::somds_v2_gateway[support=optional]
sdpi_support_use_case::somds_sensor_gateway[support=optional]

[role="support-use-case", use-case-id="aars", oid-arcs=.6]
===== <<label_use_case_name_aars>> (<<acronym_aars>>)
This use case provides capabilities for requirements from  <<clause_use_case_aars>>.
...
* *Technical Pre-Conditions*:  <<acronym_aars>> <<clause_use_case_aars_technical_precondition>> are fully supported by SDPi-P
* *Scenarios*:  <<acronym_aars>> <<clause_use_case_aars_scenarios>> basic communication requirements are supported by SDPi-P
...
In addition, SES requirements from <<ref_ieee_11073_10700_2022>> should be fully applied to this technical framework element.
...
[NORMATIVE]
====
A <<vol1_spec_sdpi_p_actor_somds_participant>> should use TLS 1.3, or a higher version.
====
****

Additional information and requirements may be provided in <<vol2_appendix_c_security_management>> or <<vol2_appendix_a_mdpws_messages>>.
