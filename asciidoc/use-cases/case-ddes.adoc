[#clause_use_case_ddes,sdpi_offset=5]
...
A local hospital is in the middle of a new <<acronym_ehr>> rollout.  Joe Furst is responsible for integrating data from their ICU devices (patient monitors, ventilators, infusion devices, etc.) with the new <<acronym_ehr>>.  Once they are done, the data from the devices will be reviewed/validated by the ICU clinicians and then automatically entered into the patient’s clinical record.
...
[#clause_use_case_ddes_technical_precondition]
...
[#clause_use_case_ddes_scenarios]
==== Scenarios
